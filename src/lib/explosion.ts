...
export const explode = (x: number, y: number) => {
  ...
  c.style.zIndex = 100 as unknown as string
  c.width = 200 * ratio
  c.height = 200 * ratio
...
const r = (a: number, b: number, c: boolean = false) =>
  parseFloat((Math.random() * ((a ? a : 1) - (b ? b : 0)) + (b ? b : 0)).toFixed(c ? 1 : 0))
